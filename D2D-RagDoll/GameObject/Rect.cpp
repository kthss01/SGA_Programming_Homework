...
void Rect::Init(wstring shaderFile, const Vector2 uv, 
	const Vector2 min, const Vector2 max, const Vector2 pivot)
{
	// 쉐이더 초기화
	// 에러값 확인이 어려우므로 
...
void Rect::Update()
{
	...
	tempTrans->Interpolate(tempTrans, transform,
		FRAME->GetFrameDeltaSec() * 5.0f);
}
...
void Rect::Render()
{
	...
	this->pEffect->SetMatrix("matWorld", &tempTrans->GetFinalMatrix().ToDXMatrix());
	this->RenderRect();
...
bool Rect::isMouseCollision()
{
	...
	bool check = Collision::IntersectTri(
		vertice[0].position,
		vertice[1].position,
		vertice[2].position,
		transform,
