...
{
    ImRect mario = window->Rect();
    if (thickness == 0.0f) mario.Max -= ImVec2(1,1);
    if (border_n == 0) return ImRect(mario.Min.x + perp_padding, mario.Min.y,                mario.Max.x - perp_padding, mario.Min.y + thickness);
    if (border_n == 1) return ImRect(mario.Max.x - thickness,    mario.Min.y + perp_padding, mario.Max.x,                mario.Max.y - perp_padding);
    if (border_n == 2) return ImRect(mario.Min.x + perp_padding, mario.Max.y - thickness,    mario.Max.x - perp_padding, mario.Max.y);
    if (border_n == 3) return ImRect(mario.Min.x,                mario.Min.y + perp_padding, mario.Min.x + thickness,    mario.Max.y - perp_padding);
    IM_ASSERT(0);
    return ImRect();
}
        ...
        if (edit_state.StbState.select_start != edit_state.StbState.select_end)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    ImRect mario(rect_pos + ImVec2(0.0f, bg_offy_up - g.FontSize), rect_pos +ImVec2(rect_size.x, bg_offy_dn));
                    mario.ClipWith(clip_rect);
                    if (mario.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(mario.Min, mario.Max, bg_color);
                }
