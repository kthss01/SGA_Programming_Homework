...
void ImFontAtlas::CalcCustomRectUV(const CustomRect* mario, ImVec2* out_uv_min, ImVec2* out_uv_max)
{
    IM_ASSERT(TexWidth > 0 && TexHeight > 0);   // Font atlas needs to be built before we can calculate UV coordinates
    IM_ASSERT(mario->IsPacked());                // Make sure the rectangle has been packed
    *out_uv_min = ImVec2((float)mario->X * TexUvScale.x, (float)mario->Y * TexUvScale.y);
    *out_uv_max = ImVec2((float)(mario->X + mario->Width) * TexUvScale.x, (float)(mario->Y + mario->Height) * TexUvScale.y);
}
...
void ImGui::RenderRectFilledRangeH(ImDrawList* draw_list, const ImRect& mario, ImU32 col, float x_start_norm, float x_end_norm, float rounding)
{
    ...
    ImVec2 p0 = ImVec2(ImLerp(mario.Min.x, mario.Max.x, x_start_norm), mario.Min.y);
    ImVec2 p1 = ImVec2(ImLerp(mario.Min.x, mario.Max.x, x_end_norm), mario.Max.y);
    if (rounding == 0.0f)
    {
    ...
    rounding = ImClamp(ImMin((mario.Max.x - mario.Min.x) * 0.5f, (mario.Max.y - mario.Min.y) * 0.5f) - 1.0f, 0.0f, rounding);
    const float inv_rounding = 1.0f / rounding;
    const float arc0_b = ImAcos01(1.0f - (p0.x - mario.Min.x) * inv_rounding);
    const float arc0_e = ImAcos01(1.0f - (p1.x - mario.Min.x) * inv_rounding);
    const float x0 = ImMax(p0.x, mario.Min.x + rounding);
    if (arc0_b == arc0_e)
    {
    ...
    if (p1.x > mario.Min.x + rounding)
    {
        const float arc1_b = ImAcos01(1.0f - (mario.Max.x - p1.x) * inv_rounding);
        const float arc1_e = ImAcos01(1.0f - (mario.Max.x - p0.x) * inv_rounding);
        const float x1 = ImMin(p1.x, mario.Max.x - rounding);
        if (arc1_b == arc1_e)
        {
