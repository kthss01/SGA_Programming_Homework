...
// 이 방법은 아님 그려지지도 않고 그려지더라도 빈공간만 더 그려질꺼 같음
// 가능한 방법임 근데 야매인거 같기는 함 GdiTransparentBlt에서 가능함
	...
	int width, int height, BYTE alpha)
{
	...
	if (isTrans) {
		...
		// test 실패
		//StretchBlt(blendImage->hMemDC, 0, 0, width, height,
		//	m_imageInfo->hMemDC, 0, 0, m_imageInfo->width, m_imageInfo->height,
		//	SRCCOPY);
		//BitBlt(m_imageInfo->hMemDC, 0, 0, width, height,
		//	blendImage->hMemDC, 0, 0, SRCCOPY); // 고속 복사 SRCCOPY
		...
		//StretchBlt(tempDC, 0, 0, width, height, m_imageInfo->hMemDC,
		//	0, 0, m_imageInfo->width, m_imageInfo->height, SRCCOPY);
		BitBlt(tempDC, 0, 0, width, height,
			hdc, destX, destY, SRCCOPY); // 고속 복사 SRCCOPY

		// 1. 출력 해야될 DC에 그려져 있는 내용을 블렌드 이미지에 그려준다
		//BitBlt(blendImage->hMemDC, 0, 0, m_imageInfo->width, m_imageInfo->height,
		//	hdc, destX, destY, SRCCOPY); // 고속 복사 SRCCOPY
...
void Image::AlphaRender2(HDC hdc, int destX, int destY, int width, int height, BYTE alpha)
{
	// 투명도를 얼마만큼 주겠느냐 설정
	blendFunc.SourceConstantAlpha = alpha;
	// 마젠타 지우고 할 꺼면
